...
class StrandFilter(object):
    ...
    def __call__(self, lr_strand_a, lr_strand_b, compare_strand_a, compare_strand_b):
        if (compare_strand_a, compare_strand_b) in self.allowable_strands:
            return True
        else:
            return False

class BndFilter(StrandFilter):
    def __call__(self, lr_strand_a, lr_strand_b, compare_strand_a, compare_strand_b):
        '''
        Long read validation strands are different than typical SV BEDPE. SV
        VCF +- is ++ in long read bedpe
        '''
        # All strand orientations are allowable, but they have to be the SAME
        # type e.g. lr = ( '-', '-' ) should be ok with compare = ('+', '-')
        return (lr_strand_a == lr_strand_b) != (compare_strand_a == compare_strand_b)



class LongReadIntersect(object):
    def __init__(self):
        self.filters = {
                'DEL': StrandFilter([('+', '-')]),
                'MEI': StrandFilter([('+', '-')]),
    ...
    def __call__(self, onekg_type, lr_strand_a, lr_strand_b, compare_strand_a, compare_strand_b):
        try:
            return self.filters[onekg_type](lr_strand_a, lr_strand_b, compare_strand_a, compare_strand_b)
        except KeyError:
